...
var changeKey = function() {
  ...
  output.sendMessage([176, 123, 0 ]); // all notes off
};
...
var push = function(obj, callback) {
  ...
  output.sendMessage([144,note,50 + Math.floor(Math.random()*60)]); // note start
  setTimeout(function() {
    output.sendMessage([128,note,0]); // note end
  }, 100+ Math.floor(Math.random()*4000))
  callback();
}
